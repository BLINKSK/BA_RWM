    ...
    with tf.Graph().as_default() as graph:  
        ...
        with tf.compat.v1.Session(graph=graph) as sess:
            ...
            for i in tqdm(range(num_test_nor)): 
                ...
                # batch_x_nor = np.squeeze(batch_x_nor)
                batch_x_nor = (batch_x_nor * 255).astype(np.uint8)
                # print(batch_x_nor)
                batch_x_nor = preprocess_input(batch_x_nor)
                # batch_x_nor = np.expand_dims(batch_x_nor, axis=0) 
            ...
            for i in tqdm(range(num_test_poi)): 
                ...
                # batch_x_poi = np.squeeze(batch_x_poi)
                batch_x_poi = (batch_x_poi * 255).astype(np.uint8)
                batch_x_poi = preprocess_input(batch_x_poi)
                # batch_x_poi = np.expand_dims(batch_x_poi, axis=0)
