...
def evaluate(trained_model, backdoor):
    if trained_model is not None:
        ...
        model = NASNetMobile(weights='imagenet', include_top=True, input_shape=(224, 224, 3))
    model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy'])
    print('##########   evaluate model   ##########', trained_model)
    # model.summary()
    # model.save('four_models/NASNetMobile/pretrained_model.h5')
...
def re_train(model_path):
    BATCH_SIZE = 500
    EPOCH = 20
    saved_path = 'four_models/NASNetMobile' 
...
re_train('four_models/NASNetMobile/normal_model.h5')
evaluate('four_models/NASNetMobile/retrain_model_e20.h5', True)
evaluate('four_models/NASNetMobile/retrain_model_final.h5', True)
